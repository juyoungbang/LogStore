...
// Use the db whose last level is in the following location
static const char* FLAGS_ll_db = NULL;
...
// Should opened tables hint to the file system that they're random
static bool FLAGS_advise_random_on_open = true;
...
class Benchmark {
 ...
 private:
  ...
  void Open() {
    ...
    options.last_level = std::string(FLAGS_ll_db);
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_db = argv[i] + 5;
    } else if (strncmp(argv[i], "--ll_db=", 8) == 0) {
      FLAGS_ll_db = argv[i] + 8;
  ...
  if (FLAGS_db == NULL) {
      ...
      FLAGS_ll_db = FLAGS_db = default_db_path.c_str();
  }

  if (FLAGS_ll_db == NULL) {
    fprintf(stderr, "You must specify where the last level location is\n");
    exit(1);
  }
